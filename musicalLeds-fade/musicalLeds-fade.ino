  ...
  for(int i=0; i < 6; i++) { // bucket index
    for(int n=0; n < 10; n++) { // quantity of LEDs
      if(n < inputArray[i]) {
        ...
        strip.setPixelColor(LED[i][n], 175, 79, 0);
      }
      else if(n < minusTwo[i]) {
        strip.setPixelColor(LED[i][n], 0, 105, 105);
      }
      else {
        strip.setPixelColor(LED[i][n], 0);
      }
    }
  }

  //  set minusTwo array equal to minusOne
  memcpy( minusTwo, minusOne, 6 );
  //  set minusOne array equal to inputArray
  memcpy( minusOne, inputArray, 6 );
}  
